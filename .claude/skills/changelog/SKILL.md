...
Drafts a new MDX entry in `content/changelog/` for a specific sprint.
...
Save to `content/changelog/<filename>.mdx`.
...
If the user provides a sprint recap, read it first — it has done a lot of the engineering-to-merchant translation. The recap is the better starting point than raw issues; raw issues are the fallback.
