...
Confirm Shop Sync shows a green **Connected** badge, Shopify products have SKU codes, and your fulfillment location is created in the Shop Sync settings. If Shopify will handle fulfillment and customer communication, keep the matching NEXT notifications off (they start disabled on new stores, or **Disable** them if they are Active) and enable the notices your team still wants NEXT to send.
...
\* Customer notifications start disabled on new stores. Merchants using [**Shop Sync**](/docs/apps/shop-sync) to send Next Commerce orders to Shopify for fulfillment and customer communication can leave the notifications Shopify sends turned off, or **Disable** them on older stores where they are still Active.
