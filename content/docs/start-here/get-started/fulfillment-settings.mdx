...
Set up **Allowed Destination Countries** to define which countries this Location can serve, and to give it priority routing for those countries.  This field is the highest-priority input to [**Location-Based Routing**](/docs/features/fulfillment-guide/location-based-routing) — leaving it empty means the Location serves every country but isn't prioritized for any.  Note: the Location's name (e.g. "US Warehouse") has no effect on routing; only the Allowed Destinations list does.
