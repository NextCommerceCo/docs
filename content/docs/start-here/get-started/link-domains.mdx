...
**Using Campaigns?** Storefront domains and Campaign domains are configured in different places. This page covers **storefront domains** — used for checkout links, PayPal, Apple Pay, and your store's public-facing pages. To add or change a domain for a Campaign funnel, go to **Apps > Campaigns App > Settings > Add Domain**. See [**Campaigns App Domain Allowlist**](/docs/apps/campaigns-app#configure-campaigns-app) for details.
...
Every store requires a Primary Domain before live commerce can be enabled. If you also send customer notifications from NEXT, set up and verify an Email Sending Domain so transactional emails use your brand domain.

</Callout>

The Primary Domain is used for [**Checkout Links**](/docs/manage/customers/checkout-links) created from customer carts via the Dashboard, as well as other links to your storefront, including express and redirect payment methods such as [**PayPal**](/docs/features/payments/paypal) and [**Apple Pay**](/docs/features/payments/apple-pay).
