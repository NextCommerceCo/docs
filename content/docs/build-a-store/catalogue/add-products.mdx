...
Confirm each sellable product has a title, price, image, public status, and the correct shipping setting. Physical products should also have SKUs so fulfillment, reporting, and integrations can identify the item reliably.
...
* **Tax Class -** assign a [**Tax Class**](/docs/build-a-store/catalogue/tax-settings) to individual SKUs when tax is enabled
...
Adding subscription-specific prices is useful for [**subscription management**](/docs/manage/subscriptions-guide/managing-subscriptions) scenarios by your store's customer service and sales reps.

</Callout>

### Add Product Content

Add content to customize your product page on the storefront.  Switch to HTML view to add custom HTML.  If your store supports [**multiple languages**](/docs/start-here/get-started/add-locations-and-languages#additional-languages), you may input translated content via the language tabs:

* **Title** - the name of the product
* **Description** - a longer form description of the product

The product URL slug is generated from the title and stays in sync when the title changes, so updating a product name will also keep the URL path aligned unless you override it elsewhere in the product editor.

#### Images

Upload one or more images of your product.  When creating [**Product Variants**](/docs/build-a-store/catalogue/product-variants) you can choose to assign one of the series of images to each variant, for example to display a certain size or color option for a variant.

#### Product Details

Set up the following product details:

* **Is Discountable -** set to allow the product to be discounted via [**Offers**](/docs/features/offers) & [**Coupons**](/docs/features/offers/coupons)
...
If you've set up [**Categories**](/docs/build-a-store/catalogue/add-products#product-categories) you may assign one or more to your product from this view.
...
* **Tax Code -** optionally assign a [**Tax Code**](/docs/build-a-store/catalogue/tax-settings) to override the category default (if applicable)
...
See [**Product Variants**](/docs/build-a-store/catalogue/product-variants#understanding-product-variants) for more on how to customize attributes, variants, and more.
