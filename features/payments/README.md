...
Next Commerce supports a variety of global gateways for bankcard transactions, encompassing credit and debit cards such as Visa, Mastercard, AMEX, Discover, Maestro, JCB and more.  Many regional alternative payment methods are also supported.
...
* (Optional) - Enable 3DS2 support, and assign a[ **3DS Merchant Profile**](3ds2-payments.md#id-3ds2-merchant-profiles) to the gateway
* (Optional) - Enable Cascading [**Declined Initial Charge Attempts**](payment-failure-cascading.md)
* (Optional) - for supported gateways - specify a Statement Descriptor to override your payment provider's default descriptor

{% hint style="info" icon="circle-bolt" %}
For orders created via the [**Admin API**](https://developers.nextcommerce.com/docs/admin-api/guides/external-checkout/), you may optionally define a **`payment_gateway`** ID to route the transaction to a specific gateway ID via the  **`payment_details > payment_gateway`** parameter.
{% endhint %}

### Gateway Groups

Gateway Groups allow the orchestration of transactions across multiple gateways on a proportional basis.  Assign one or multiple gateways to a group, and define a weighting to each gateway to prioritize the distribution of transactions.
...
On the **Add Gateway Group** card, assign one or more existing gateways to the group, and choose a weight for the proportion of transactions that should be directed to each of them, with the higher weights receiving the higher proportion of volume.&#x20;
...
* then on the weighting assigned to the eligible gateways. &#x20;
...
{% hint style="info" icon="circle-bolt" %}
For orders created via the [**Admin API**](https://developers.nextcommerce.com/docs/admin-api/guides/external-checkout/), you may pass a **`payment_gateway_group`** ID in the **`payment_details > payment_gateway_group`** parameter.  If a payment gateway group ID is specified, it will override the default gateway, or any value input into the **`payment_gateway`** field.

[**Campaigns App**](../../apps/campaigns-app/) includes a gateway group setting to automatically choose the gateway group on each campaign.
{% endhint %}
